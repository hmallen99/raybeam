...
int main () {
    ...
    vector<std::shared_ptr<vec3>> vecList = {
        std::make_shared<vec3>(-2, 1, -1),
        std::make_shared<vec3>(2, 1, -1),
        std::make_shared<vec3>(2, -1, -1),
        std::make_shared<vec3>(-2, -1, -1),
        std::make_shared<vec3>(-2, -1, -2),
        std::make_shared<vec3>(-2, 1, -2),
        std::make_shared<vec3>(2, 1, -2),
        std::make_shared<vec3>(2, -1, -2)
    };
    material* green = new lambertian(std::make_shared<vec3>(0, 1, 0));
    material* blue = new lambertian(std::make_shared<vec3>(0, 0, 1));
    material* red = new lambertian(std::make_shared<vec3>(1, 0, 0));
    material* yellow = new lambertian(std::make_shared<vec3>(1, 1, 0));
    material* white = new lambertian(std::make_shared<vec3>(1, 1, 1));
    ...
    rt->addObject(new sphere(make_shared<vec3>(-1.0, 0, -1), 0.5, blue));
